...
public static class GeomUtil {
    ...
    /// <summary> 点到直线的最小距离（垂足会超出线段） </summary>
    public static float DistancePointLine(Vector3 point, Vector3 lineStart, Vector3 lineEnd) {
        return Vector3.Magnitude(GetPerpendicularOnLine(point, lineStart, lineEnd, out _) - point);
    }
    ...
    public static (int startIndex, int endIndex) GetClosestPolyLineToPoint(Vector3 point, Vector3[] vertices, bool isClosed) {
        ...
        for (int i = 0, len = vertices.Length; i < len; i++) {
            ...
            float distance = DistancePointLineSegment(point, lineStart, lineEnd);
    ...
    public static (int startIndex, int endIndex) GetClosestPolyLineToRayOrigin(Vector3 rayOrigin, Vector3 rayDirection, Vector3[] vertices, bool isClosed) {
        ...
        for (int i = 0, len = vertices.Length; i < len; i++) {
            ...
            if (GetTwoLineIntersection(rayOrigin, rayDirection, lineStart, lineDirection, out Vector3 intersection, out float t1, out float t2)) {
    ...
    /// 注意：使用此方法必须两直线都在同一个平面上（3D直线请使用<see cref="GetClosestPointsOnTwo3DLines"/>），交点会超出线段范围
    /// </summary>
    /// <param name="lineStart1"></param>
    /// <param name="lineEnd1"></param>
    /// <param name="lineStart2"></param>
    /// <param name="lineEnd2"></param>
    /// <param name="intersection"> 输出的交点 </param>
    ...
    public static bool GetTwoLineIntersection(Vector3 lineStart1, Vector3 lineEnd1, Vector3 lineStart2, Vector3 lineEnd2, out Vector3 intersection, out float t1, out float t2) {
        ...
        if (Mathf.Abs(planarFactor) < 0.0001f && crossVec1and2.sqrMagnitude > 0.0001f) {
            ...
            Vector3 crossVec3and1 = Vector3.Cross(lineVec3, lineDirection1);
            t2 = -(Vector3.Dot(crossVec3and1, crossVec1and2) / crossVec1and2.sqrMagnitude);
    ...
    public static bool GetTwoLineSegmentsIntersection(Vector3 lineStart1, Vector3 lineEnd1, Vector3 lineStart2, Vector3 lineEnd2, out Vector3 intersection, out float t1, out float t2) {
        ...
        if (intersect) {
            ...
            bool intersectLineSegment2 = t2 >= 0f && t2 <= 1f;
            return intersectLineSegment1 && intersectLineSegment2;
        }
        return false;
    }
